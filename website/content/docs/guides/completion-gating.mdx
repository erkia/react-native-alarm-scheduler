...
**iOS — best effort.** The ringing surface belongs to AlarmKit. `openAppOnly` adds an app-opening
secondary action, but cannot remove the system stop control. The package uses the initializer with
`stopButton` for compatibility with all iOS 26 SDKs. System-owned close or stop affordances may
not invoke the package's App Intents.
